...
function setup() {
    ...
    for (let r = 0; r < GDP_data.getRowCount(); r++) {
        ...
        if (isThere(currentCountryCODE)){
            ...
            if ( currentCountry.myCode === "DEU") currentCountry.myColor = color(200,200,100,150);
            if ( currentCountry.myCode === "USA") currentCountry.myColor = color(100,100,255);
            if ( currentCountry.myCode === "CHN") currentCountry.myColor = color(200,100,100);
            if ( currentCountry.myCode === "EUU") currentCountry.myColor = color(50,100,100);
            if ( currentCountry.myCode === "LCN") currentCountry.myColor = color(150,100,100);
...
function draw() {
    ...
    // Use sort function to sort the array of countries by highest GDP.
    arrayOfCountries = arrayOfCountries.sort((b, a) => parseFloat(a.arrayOfData[currentYear].y) - parseFloat(b.arrayOfData[currentYear].y))
